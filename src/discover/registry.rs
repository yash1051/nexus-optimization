...
pub fn rewrite_command(
    ...
    let has_compound = trimmed.contains("&&")
        ...
        || trimmed.contains('|')
        || trimmed.contains(" & ");
    if !has_compound && (trimmed.starts_with("nexus") || trimmed == "nexus") {
...
fn rewrite_segment_inner(
    ...
    if !env_prefix.is_empty() {
        ...
        if env_prefix.contains("RTK_DISABLED=") {
            eprintln!(
                "[nexus] RTK_DISABLED=1 detected — skipping filter for this command. \
    ...
    if cmd_part.starts_with("nexus") || cmd_part == "nexus" {
        return Some(trimmed.to_string());
    }
