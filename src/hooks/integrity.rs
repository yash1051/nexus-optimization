...
//! - Manual verification via `nexus verify`
//!
...
pub enum IntegrityStatus {
    ...
    Verified,
    /// Hash mismatch — hook has been modified outside of `nexus init`
...
/// Run integrity check and print results (for `nexus verify` subcommand)
pub fn run_verify(verbose: u8) -> Result<()> {
    ...
    match verify_hook_at(&hook_path)? {
        ...
        IntegrityStatus::Tampered { expected, actual } => {
            ...
            eprintln!();
            eprintln!("  The hook file has been modified outside of `nexus init`.");
            eprintln!("  This could indicate tampering or a manual edit.");
            eprintln!();
            eprintln!("  To restore: nexus init -g --auto-patch");
        ...
        IntegrityStatus::NoBaseline => {
            ...
            println!("      Run `nexus init -g` to establish baseline.");
        }
        ...
        IntegrityStatus::OrphanedHash => {
            ...
            eprintln!("      Run `nexus init -g` to reinstall.");
        }
    }
...
/// re-run `nexus init -g --auto-patch` to re-establish the baseline.
pub fn runtime_check() -> Result<()> {
    ...
    match verify_hook_at(&hook_path)? {
        ...
        IntegrityStatus::Tampered { expected, actual } => {
            eprintln!("nexus: hook integrity check FAILED");
            eprintln!(
                "  Expected hash: {}...",
            ...
            eprintln!();
            eprintln!("  To restore:  nexus init -g --auto-patch");
            eprintln!("  To inspect:  nexus verify");
            std::process::exit(1);
        }
        IntegrityStatus::OrphanedHash => {
            eprintln!("nexus: warning: hash file exists but hook is missing");
            eprintln!("  Run `nexus init -g` to reinstall.");
