...
/// Template written by `nexus init` when no filters.toml exists yet.
...
const RTK_MD: &str = "NEXUS.md";
#[allow(dead_code)]
const LEGACY_RTK_MD: &str = "RTK.md";
const CLAUDE_MD: &str = "CLAUDE.md";
const AGENTS_MD: &str = "AGENTS.md";
const RTK_MD_REF: &str = "@NEXUS.md";
#[allow(dead_code)]
const LEGACY_RTK_MD_REF: &str = "@NEXUS.md";
...
pub fn uninstall(
    ...
    if rtk_md_path.exists() {
        if dry_run {
            println!("[dry-run] would remove NEXUS.md: {}", rtk_md_path.display());
        } else {
            fs::remove_file(&rtk_md_path)
                .with_context(|| format!("Failed to remove NEXUS.md: {}", rtk_md_path.display()))?;
        }
        removed.push(format!("NEXUS.md: {}", rtk_md_path.display()));
    }

    // 3. Remove @NEXUS.md reference from CLAUDE.md
    ...
    if claude_md_path.exists() {
        ...
        if working_content.contains(RTK_MD_REF) {
            ...
            removed.push("CLAUDE.md: removed @NEXUS.md reference".to_string());
        }
...
fn uninstall_codex_at(codex_dir: &Path, ctx: InitContext) -> Result<Vec<String>> {
    ...
    if rtk_md_path.exists() {
        if dry_run {
            ...
            fs::remove_file(&rtk_md_path)
                .with_context(|| format!("Failed to remove NEXUS.md: {}", rtk_md_path.display()))?;
            if verbose > 0 {
                eprintln!("Removed NEXUS.md: {}", rtk_md_path.display());
            }
        }
        removed.push(format!("NEXUS.md: {}", rtk_md_path.display()));
    }
    ...
    if remove_rtk_reference_from_agents(
        ...
        removed.push("AGENTS.md: removed @NEXUS.md reference".to_string());
    }
...
/// Used when removing @NEXUS.md line from CLAUDE.md
...
/// Default mode: hook + slim RTK.md + @NEXUS.md reference
fn run_default_mode(
    global: bool,
    patch_mode: PatchMode,
    install_opencode: bool,
    ...
    // 3. Patch CLAUDE.md (add @NEXUS.md, migrate if needed)
    ...
    if !dry_run {
        ...
        println!("  NEXUS.md:    {} (10 lines)", rtk_md_path.display());
        if let Some(path) = &opencode_plugin_path {
            println!("  OpenCode:  {}", path.display());
        }
        println!("  CLAUDE.md: @NEXUS.md reference added");

        if migrated {
            println!("\n  [ok] Migrated: removed 137-line RTK block from CLAUDE.md");
            println!("              replaced with @NEXUS.md (10 lines)");
        }
    }
...
/// Hook-only mode: just the hook, no NEXUS.md
...
fn run_claude_md_mode(global: bool, install_opencode: bool, ctx: InitContext) -> Result<()> {
    ...
    let recovery_cmd = if global {
        "nexus init -g --claude-md"
    } else {
        "nexus init --claude-md"
    };
...
fn run_codex_mode_with_paths(
    ...
    // ISSUE #892: In global mode, use absolute path so @NEXUS.md resolves
    ...
    if !dry_run {
        ...
        println!("  NEXUS.md:    {}", rtk_md_path.display());
        if added_ref {
...
/// Patch CLAUDE.md: add @NEXUS.md, migrate if old block exists
fn patch_claude_md(path: &Path, ctx: InitContext) -> Result<bool> {
    ...
    // Check if @NEXUS.md already present
    if content.contains(RTK_MD_REF) {
        if verbose > 0 {
            eprintln!("@NEXUS.md reference already present in CLAUDE.md");
        }
        if migrated {
            if dry_run {
                println!(
    ...
    if dry_run {
        println!(
            "[dry-run] would add @NEXUS.md reference to CLAUDE.md: {}",
            path.display()
        );
        if verbose > 0 {
            println!("[dry-run] content:\n{}", new_content);
        ...
        if verbose > 0 {
            eprintln!("Added @NEXUS.md reference to CLAUDE.md");
        }
    }
...
/// Patch AGENTS.md: add @NEXUS.md (or absolute path), migrate old inline block if present
fn patch_agents_md(path: &Path, rtk_md_ref: &str, ctx: InitContext) -> Result<bool> {
    ...
    // ISSUE #892: Check for both relative and absolute @NEXUS.md references
    if content.contains(RTK_MD_REF) || content.contains(rtk_md_ref) {
        if verbose > 0 {
            eprintln!("{} reference already present in AGENTS.md", rtk_md_ref);
        }
        // ISSUE #892: Migrate old relative @NEXUS.md to absolute path if needed
...
fn remove_rtk_reference_from_agents(path: &Path, refs: &[&str], ctx: InitContext) -> Result<bool> {
    ...
    if dry_run {
        println!(
            "[dry-run] would remove NEXUS.md reference from AGENTS.md: {}",
            path.display()
        );
        if verbose > 0 {
            println!("[dry-run] content:\n{}", cleaned);
    ...
    if verbose > 0 {
        eprintln!(
            "Removed NEXUS.md reference from AGENTS.md: {}",
            path.display()
        );
    }
...
/// Remove Cursor Nexus artifacts: hook script + hooks.json entry
...
fn show_claude_config() -> Result<()> {
    ...
    if binary_hook_registered {
        ...
        #[cfg(unix)]
        {
            ...
            if !is_executable {
                ...
                println!(
                    "[warn] Hook: {} (outdated — run `nexus init -g` to upgrade to native binary)",
                    hook_path.display()
                );
            } else if is_executable && has_guards {
                println!(
                    "[warn] Hook: {} (legacy script v{} — run `nexus init -g` to upgrade)",
                    hook_path.display(),
                    hook_version
                );
            } else {
        ...
        {
            println!(
                "[warn] Hook: {} (legacy script — run `nexus init -g` to upgrade)",
                hook_path.display()
            );
        }
    } else {
    ...
    // Check RTK.md
    if rtk_md_path.exists() {
        println!("[ok] NEXUS.md: {} (slim mode)", rtk_md_path.display());
    } else {
        println!("[--] NEXUS.md: not found");
    }
        ...
        if cursor_binary_registered {
            ...
            #[cfg(unix)]
            {
                ...
                if !is_executable {
                    ...
                    println!(
                        "[warn] Cursor hook: {} (legacy script — run `nexus init -g --agent cursor` to upgrade)",
                        cursor_hook.display()
                    );
                }
            }
            ...
            #[cfg(not(unix))]
            {
                println!("[warn] Cursor hook: {} (legacy script — run `nexus init -g --agent cursor` to upgrade)", cursor_hook.display());
            }
        } else {
    ...
    println!("\nUsage:");
    println!("  nexus init              # Full injection into local CLAUDE.md");
    println!("  nexus init -g           # Hook + NEXUS.md + @NEXUS.md + settings.json (recommended)");
    println!("  nexus init -g --auto-patch    # Same as above but no prompt");
    println!("  nexus init -g --no-patch      # Skip settings.json (manual setup)");
    println!("  nexus init -g --uninstall     # Remove all Nexus artifacts");
    println!("  nexus init -g --claude-md     # Legacy: full injection into ~/.claude/CLAUDE.md");
    println!("  nexus init -g --hook-only     # Hook only, no NEXUS.md");
    println!("  nexus init --codex            # Configure local AGENTS.md + NEXUS.md");
    println!("  nexus init -g --codex         # Configure $CODEX_HOME/AGENTS.md + $CODEX_HOME/NEXUS.md (or ~/.codex/)");
    println!("  nexus init -g --opencode      # OpenCode plugin only");
    println!("  nexus init -g --agent cursor  # Install Cursor Agent hooks");
...
fn show_codex_config() -> Result<()> {
    ...
    println!("Nexus Configuration (Codex CLI):\n");

    if global_rtk_md.exists() {
        println!("[ok] Global NEXUS.md: {}", global_rtk_md.display());
    } else {
        println!("[--] Global NEXUS.md: not found");
    }
    ...
    if local_rtk_md.exists() {
        println!("[ok] Local NEXUS.md: {}", local_rtk_md.display());
    } else {
        println!("[--] Local NEXUS.md: not found");
    }

    if local_agents_md.exists() {
        let content = fs::read_to_string(&local_agents_md)?;
        if has_rtk_reference(&content, &[RTK_MD_REF]) {
            println!("[ok] Local AGENTS.md: @NEXUS.md reference");
        } else if content.contains(RTK_BLOCK_START) {
    ...
    println!("  nexus init --codex              # Configure local AGENTS.md + NEXUS.md");
    println!("  nexus init -g --codex           # Configure $CODEX_HOME/AGENTS.md + $CODEX_HOME/NEXUS.md (or ~/.codex/)");
    println!("  nexus init -g --codex --uninstall  # Remove global Codex Nexus artifacts");
...
/// Entry point for `nexus init --gemini`
pub fn run_gemini(
    global: bool,
    hook_only: bool,
    patch_mode: PatchMode,
...
mod tests {
    ...
    fn test_init_is_idempotent() {
        ...
        let count = content.matches("@NEXUS.md").count();
        assert_eq!(count, 1);
    }
    ...
    fn test_patch_agents_md_adds_reference_once() {
        ...
        assert_eq!(content.matches("@NEXUS.md").count(), 1);
    }
    ...
    fn test_run_codex_mode_dry_run_writes_nothing() {
        ...
        assert!(
            !rtk_md.exists(),
            "dry-run must not create NEXUS.md: {}",
            rtk_md.display()
        );
        assert!(
            !agents_md.exists(),
    ...
    fn test_uninstall_dry_run_preserves_artifacts() {
        ...
        with_claude_dir_override(&tmp, |claude_dir| {
            ...
            assert!(
                claude_dir.join(RTK_MD).exists(),
                "dry-run uninstall must not remove NEXUS.md"
            );
            assert!(
                claude_dir.join(SETTINGS_JSON).exists(),
    ...
    fn test_uninstall_handles_both_artifacts() {
        ...
        let after_at_removal: String = content
            .lines()
            .filter(|line| !line.trim().starts_with("@NEXUS.md"))
            .collect::<Vec<_>>()
            .join("\n");

        assert!(!after_at_removal.contains("@NEXUS.md"));
