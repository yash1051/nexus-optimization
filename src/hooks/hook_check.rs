...
pub fn status() -> HookStatus {
    ...
    if binary_hook_registered(&claude_dir) {
        ...
        // (migration not complete — user should run `nexus init -g` to clean up)
...
fn check_and_warn() -> Option<()> {
    let warning = match status() {
        ...
        HookStatus::Missing => {
            "[nexus] /!\\ No hook installed — run `nexus init -g` for automatic token savings"
        }
        HookStatus::Outdated => "[nexus] /!\\ Hook outdated — run `nexus init -g` to update",
    };
