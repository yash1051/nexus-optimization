...
//! - `nexus trust` stores the SHA-256 hash after user review
...
pub fn check_trust(filter_path: &Path) -> Result<TrustStatus> {
    ...
    if std::env::var("RTK_TRUST_PROJECT_FILTERS").as_deref() == Ok("1") {
        ...
        eprintln!(
            "[nexus] WARNING: RTK_TRUST_PROJECT_FILTERS=1 ignored (CI environment not detected)"
        );
    }
    ...
    let store = match read_store() {
        ...
        Err(e) => {
            eprintln!(
                "[nexus] WARNING: trust store unreadable ({}), treating all filters as untrusted",
                e
            );
            TrustStore::default()
        }
...
/// Run `nexus trust` — review and trust project-local filters.
