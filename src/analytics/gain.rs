...
pub fn run(
    ...
    if !daily && !weekly && !monthly && !all {
        ...
        let title = if project_scope.is_some() {
            "Nexus Token Savings (Project Scope)"
        } else {
        ...
        match hook_check::status() {
            hook_check::HookStatus::Missing => {
                eprintln!(
                    "{}",
                    "[warn] No hook installed — run `nexus init -g` for automatic token savings"
                        .yellow()
                );
                eprintln!();
            }
            hook_check::HookStatus::Outdated => {
                eprintln!(
                    "{}",
                    "[warn] Hook outdated — run `nexus init -g` to update".yellow()
                );
                eprintln!();
            }
...
fn check_rtk_disabled_bypass() -> Option<String> {
    ...
    if pct > 10.0 {
        Some(format!(
            "[warn] {} commands ({:.0}%) used RTK_DISABLED=1 unnecessarily — run `nexus discover` for details",
            bypassed, pct
        ))
    } else {
        None
