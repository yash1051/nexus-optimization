...
pub fn run_streaming(
    ...
    if is_streaming {
                ...
                if is_stderr {
                    if !capped_err {
                        if raw_stderr.len() + line.len() < RAW_CAP {
                            ...
                            capped_err = true;
                            eprintln!("[nexus] warning: stderr exceeds 10 MiB — capture truncated");
                        }
                    }
                } else if !capped_out {
                    if raw_stdout.len() + line.len() < RAW_CAP {
                        ...
                        capped_out = true;
                        eprintln!("[nexus] warning: stdout exceeds 10 MiB — filter input truncated");
                    }
                }
                ...
                FilterMode::Buffered(filter_fn) => {
                    for line in BufReader::new(stdout).lines().map_while(Result::ok) {
                        if raw_stdout.len() + line.len() < RAW_CAP {
                            ...
                            capped_out = true;
                            eprintln!(
                                "[nexus] warning: output exceeds 10 MiB — filter input truncated"
                            );
                        }
                    }
                    ...
                    .unwrap_or_else(|_| {
                        eprintln!("[nexus] warning: filter panicked — passing through raw output");
                        raw_stdout.clone()
                    });
        ...
        raw_stderr = stderr_thread.join().unwrap_or_else(|e| {
            eprintln!("[nexus] warning: stderr reader thread panicked: {:?}", e);
            String::new()
        });
    }
