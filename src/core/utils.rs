        ...
        None => {
            #[cfg(unix)]
            {
                ...
                if let Some(sig) = output.status.signal() {
                    eprintln!("[nexus] {}: process terminated by signal {}", label, sig);
                    return 128 + sig;
                }
            }
            eprintln!("[nexus] {}: process terminated by signal", label);
            1
        }
    }
}
...
pub fn fallback_tail(output: &str, label: &str, n: usize) -> String {
    eprintln!(
        "[nexus] {}: output format not recognized, showing last {} lines",
        label, n
    );
    ...
    match resolve_binary(name) {
        ...
        Err(e) => {
            ...
            if cfg!(any(target_os = "windows", debug_assertions)) {
                eprintln!(
                    "nexus: Failed to resolve '{}' via PATH, falling back to direct exec: {}",
                    name, e
                );
            }
