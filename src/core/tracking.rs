...
///
/// Serializable to JSON for export via `nexus gain --daily --format json`.
///
/// # JSON Schema
///
/// ```json
...
///
/// Serializable to JSON for export via `nexus gain --weekly --format json`.
...
///
/// Serializable to JSON for export via `nexus gain --monthly --format json`.
#[derive(Debug, Serialize)]
...
impl Tracker {
    ...
    /// Get parse failure summary for `nexus gain --failures`.
