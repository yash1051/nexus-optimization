...
#[command(
    name = "nexus",
    version,
    about = "Nexus - Snip noisy command output before it hits your AI",
...
enum Commands {
    ...
    /// Initialize nexus instructions for assistant CLI usage
    Init {
        ...
        /// Remove Nexus artifacts for the selected assistant mode
        #[arg(long)]
        uninstall: bool,

        /// Target Codex CLI (uses AGENTS.md + NEXUS.md, no Claude hook patching)
        #[arg(long)]
        codex: bool,
    ...
    /// Claude Code economics: spending (ccusage) vs savings (nexus) analysis
    CcEconomics {
    ...
    /// Discover missed Nexus savings from Claude Code history
    Discover {
    ...
    /// Show Nexus adoption across Claude Code sessions
    Session {},
    ...
    /// Rewrite a raw command to its Nexus equivalent (single source of truth for hooks)
    ///
    ...
    Rewrite {
        ...
        /// Accepts multiple args: `nexus rewrite ls -al` is equivalent to `nexus rewrite "ls -al"`
        #[arg(trailing_var_arg = true, allow_hyphen_values = true)]
        args: Vec<String>,
    },
...
fn run_fallback(parse_error: clap::Error) -> Result<i32> {
    ...
    // e.g. `nexus gain --badtypo` should show Clap's error, not try to run `gain` from $PATH.
    ...
    if let Some(filter) = toml_match {
        ...
        match result {
            Ok(output) => {
                ...
                // error_only: suppress output on success, show only on failure
                let display = if filter.error_only && output.status.success() {
                    filter
                        .on_empty
                        .clone()
    ...
    match command {
        PnpmCommands::Typecheck { .. } => {
            ...
            if !filters.is_empty() {
                ...
                let msg = format!(
                    "[nexus] warning: --filter is not yet supported for pnpm {}, filters preceding the subcommand will be ignored",
                    cmd_name
                );
                return Some(msg);
            }
...
mod tests {
    ...
    fn test_rewrite_clap_multi_args() {
        // This is the bug KuSh reported: `nexus rewrite ls -al` failed because
    ...
    fn test_rewrite_clap_quoted_single_arg() {
        // Quoted form: `nexus rewrite "git status"` — single arg containing spaces
    ...
    fn test_pnpm_typecheck_with_filters() {
        ...
        match cli.command {
            Commands::Pnpm { filter, command } => {
                ...
                assert_eq!(filter, vec!["@app1", "@app2"]);
                assert_eq!(warning, "[nexus] warning: --filter is not yet supported for pnpm tsc, filters preceding the subcommand will be ignored")
            }
            _ => panic!("Expected Pnpm Build command"),
        }
    }
